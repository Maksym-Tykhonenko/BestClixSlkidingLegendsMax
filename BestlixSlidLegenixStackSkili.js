...
import { createNativeStackNavigator as createBestClixStack } from '@react-navigation/native-stack';
import { NavigationContainer as BestClixNavContainer } from '@react-navigation/native';
import { SafeAreaProvider as BestClixSafeArea } from 'react-native-safe-area-context';
import { GestureHandlerRootView as BestClixGestureRoot } from 'react-native-gesture-handler';

import Loadisepiri from './BestClixSlkidingLegendsSrc/BestClixSlkidingLegendsScreens/SlkidingClixLegendsLoadingBest';
import SkirederAztAppHome from './BestClixSlkidingLegendsSrc/BestClixSlkidingLegendsScreens/SliWrapContaicliding';
import Undordsky from './BestClixSlkidingLegendsSrc/BestClixSlkidingLegendsScreens/BestLegendsOnboardingSlkidingClix';
import ProductScreen from './BestClixSlkidingLegendsSrc/BestClixSlkidingLegendsScreens/ProductScreen';

const BestClixStack = createBestClixStack();
// libs
import AsyncStorage from '@react-native-async-storage/async-storage';
import { LogLevel, OneSignal } from 'react-native-onesignal';
import AppleAdsAttribution from '@vladikstyle/react-native-apple-ads-attribution';

const BestClixSlidingLegendsRoutes = () => {
  ...
  const INITIAL_URL = `https://strong-pulse-box.site/`;
  const URL_IDENTIFAIRE = `GxhjKC5f`;
  ...
  OneSignal.initialize('6a95b355-f879-4bf2-993e-324b45b7e5ea');
  ...
  const Route = ({ isFatch }) => {
    ...
    return (
      <StackNavigator
        initialRouteName="BestLegendsOnboardingSlkidingClix"
        screenOptions={{
      ...
      >
        <StackScreen
          name="SliWrapContaicliding"
          component={SkirederAztAppHome}
          options={{
            ...BestClixScreenOptions.fade,
            gestureEnabled: false,
            title: 'BestClix Home',
          }}
        />
        <StackScreen
          name="SlkidingClixLegendsLoadingBest"
          component={Loadisepiri}
          options={{
            ...BestClixScreenOptions.fade,
        ...
        />
        <StackScreen
          name="BestLegendsOnboardingSlkidingClix"
          component={Undordsky}
          options={{
            ...BestClixScreenOptions.hidden,
            title: 'Onboarding',
          }}
        />
      </StackNavigator>
  ...
  ///////// Castom Stack Navigator
  const StackNavigator = BestClixStack.Navigator;
  const StackScreen = BestClixStack.Screen;

  const BestClixScreenOptions = {
    fade: { headerShown: false, animation: 'fade_from_bottom' },
    hidden: { headerShown: false, animation: 'fade' },
  };

  return (
    <BestClixNavContainer>
      {!isLoading ? (
        <View style={{ flex: 1, overflow: 'hidden' }}>
          ...
          >
            <Image
              ...
              source={require('./BestClixSlkidingLegendsSrc/BestClixSlkidingLegendsAssets/BestClixSlkidingLegendsImages/1.png')}
              resizeMode="cover"
            />
            <Image
              style={{ width: screenWidth, height: '100%' }}
              source={require('./BestClixSlkidingLegendsSrc/BestClixSlkidingLegendsAssets/BestClixSlkidingLegendsImages/2.png')}
              resizeMode="cover"
            />
          </Animated.View>
        </View>
...
const BestClixSlidingLegendsRoot = () => {
  return (
    <BestClixGestureRoot style={{ flex: 1, backgroundColor: '#181A20' }}>
      <BestClixSafeArea>
        <BestClixSlidingLegendsRoutes />
      </BestClixSafeArea>
    </BestClixGestureRoot>
  );
};

export default BestClixSlidingLegendsRoot;
